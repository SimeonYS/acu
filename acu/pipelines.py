...
class AcuPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `acu`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM acu WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO acu (date, title, link, content)"
